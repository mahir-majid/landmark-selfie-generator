...
class FluxInfuseNetPipeline(FluxControlNetPipeline):
    ...
    def __call__(
        ...
        infu_source_img_token: Optional[str] = None,
        ...
        # Token-specific identity injection: use target token for InfuseNet
        if infu_source_img_token and infu_source_img_token in prompt:
            infusenet_prompt = infu_source_img_token  # Just the target token!
            log_info(f"Using target token for InfuseNet: '{infusenet_prompt}' (from prompt: '{prompt}')")
        else:
            infusenet_prompt = prompt
            log_info(f"Using full prompt for InfuseNet: '{infusenet_prompt}' (no target token specified)")
        ...
        (
            ...
            prompt=infusenet_prompt,  # Use target token for InfuseNet
            prompt_2=prompt_2,
